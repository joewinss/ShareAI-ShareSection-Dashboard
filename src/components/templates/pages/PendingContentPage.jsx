...
import { useRefreshStats, useStats } from "@/hooks/useStatsInfo";
...
export const PendingContentPage = (props) => {
  ...
  const [googleReviewBadgeCount, setGoogleReviewBadgeCount] = useState({ totalChinese: 0, totalCount: 0, totalEnglish: 0 });
  const [othersBadgeCount, setOthersBadgeCount] = useState({ totalChinese: 0, totalCount: 0, totalEnglish: 0 });
  const [allBadgeCount, setAllBadgeCount] = useState({ totalChinese: 0, totalCount: 0, totalEnglish: 0 });
  ...
  useEffect(() => {
    setAllBadgeCount({
      totalChinese: statsData?.data?.contentSummary?.contentByLanguage?.chinese?.count || 0,
      totalCount: statsData?.data?.contentSummary?.totalPending || 0,
      totalEnglish: statsData?.data?.contentSummary?.contentByLanguage?.english?.count || 0,
      totalMalay: statsData?.data?.contentSummary?.contentByLanguage?.malay?.count || 0,
    });
  }, [statsData]);
  useEffect(() => {
    ...
    refreshSidebarStats();
  }, [page, filterGroup, refreshSidebarStats]);

  // useEffect(() => {
  // if (!router.isReady) return;
  // const tab = defaultTab || "googleReview";
  // setCurrentTab(tab);
  // setFilterGroup({ platform: tab });

  //   // Load outlet options on mount if user is masterHQ
  //   if (userIdentity === "masterHQ") {
  //     loadOutletOption();
  //   }
  // }, [router.isReady, defaultTab, userIdentity]);
  ...
  const tabItems = [
    {
      ...
      label: (
        <div className="flex items-center">
          ...
          <span className="ml-2">{t("googleReview", sourceKey.user)}</span>

          {googleReviewBadgeCount.totalCount > 0 && (
            <Badge
              count={googleReviewBadgeCount.totalCount}
              size="small"
              className="ml-2"
              title="Total Pending Google Review"
              style={{
                boxShadow: "none",
                border: "none",
              }}
    ...
    },
    {
      ...
      label: (
        <div className="flex items-center">
          ...
          <span className="ml-2">{t("others", sourceKey.user)}</span>

          {othersBadgeCount.totalCount > 0 && (
            <Badge
              count={othersBadgeCount.totalCount}
              size="small"
              className="ml-2"
              title="Total Others Pending Review"
              style={{
                boxShadow: "none",
                border: "none",
              }}
    ...
    return [
      {
        key: "approve",
        label: (
          <div className="flex items-center text-green-600" onClick={() => { setSelectedContent(template); setApproveModalOpen(true); }}>
            <CheckOutlined className="w-4 h-4 mr-2" /> {t("approve", sourceKey.user)}
          </div>
        ),
      },
      {
        key: "reject",
        label: (
          <div className="flex items-center text-red-600" onClick={() => { setSelectedContent(template); setRejectModalOpen(true); }}>
      ...
      },
      {
        key: "edit",
        label: (
          <div className="flex items-center" onClick={() => { setSelectedContent(template); setEditContentDrawerOpen(true); }}>
  ...
  return (
    <>
      <div className="pending-bg p-3">
        <div className="mb-5 flex flex-row justify-between">
          <div className="flex flex-col">
            <h1 className="text-3xl font-bold text-gray-900">
              {t("pending", sourceKey.user)}
            </h1>
            <span className="text-gray-600 mt-1">
              {t("pendingDesc", sourceKey.user)}
            </span>
          </div>
          <div></div>
        </div>
        ...
        <div className="min-h-screen bg-white from-slate-50 to-blue-50">
          <div className="max-w-full mx-auto">
            ...
            <div className="p-3">
              ...
              >
                {allContent?.map((template, index) => (
                  <ContentCardSection
                    ...
                    dropdownMenuItems={getMenuItems(template)}
                    t={t}
                  />
                ))}
              </motion.div>
              ...
              {allContent?.length != 0 && (
                <QuotaSummaryBar
                  remainingActive={allBadgeCount.totalCount}
                  enActive={allBadgeCount.totalEnglish}
                  cnActive={allBadgeCount.totalChinese}
                  myActive={allBadgeCount.totalMalay}
                />
              )}
            </div>
          </div>
      ...
      <SinglePopUpModal
        ...
        confirmBtn1={() =>
          handleEditContentStatus(selectedContent?._id, "reject")
        }
      />

      <SinglePopUpModal
        open={deleteModalOpen}
        type={"deletePendingContent"}
        closeable={true}
        extraData={selectedContent}
        onClose={() => {
        ...
        confirmBtn1={() => handleDeleteContent(selectedContent?._id)}
      />
      ...
      <EditContentDrawer
        ...
        onRefreshData={() =>
          // filterGroup.platform && getData((page - 1) * PAGE_SIZE)
          getData((page - 1) * PAGE_SIZE)
        }
      />
...
export default connect(mapStateToProps, mapDispatchToProps)(PendingContentPage);
