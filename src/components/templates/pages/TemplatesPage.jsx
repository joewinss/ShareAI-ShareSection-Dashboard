...
import { Plus, Edit, FileText, QrCodeIcon, CheckSquare, } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Col, message, Pagination, Row, } from "antd";
import { useTranslation } from "@/locales/useTranslation";
import { FilterIcon, GoogleReview, OthersIcon } from "../../../../public/assets/index";
...
export const TemplatesPage = (props) => {
  ...
  async function downloadQrCode() {
    const businessName = user?.businessInfo?.businessName?.trim() || user?.username || "qrcode";
  ...
  const getMenuItems = (template) => {
    // If no permission, return an empty list or null depending on your component requirement
    ...
    return [
      ...
      // },
      {
        key: "reject",
        label: (
          <div className="flex items-center cursor-pointer text-red-600" onClick={() => {
            setSelectedContent(template);
            setRejectModalOpen(true);
          }}>
            <CloseOutlined className="w-4 h-4 mr-2" />
            {t("reject", sourceKey.user)}
          </div>
        ),
      },
      {
        ...
        <div className="mb-5 flex flex-row justify-between">
          <div className="flex flex-col">
            <h1 className="text-3xl font-bold text-gray-900">
              {t("shareAI", sourceKey.user)}
            </h1>
            <span className="text-gray-600 mt-1">
              {t("shareAIDesc", sourceKey.user)}
            </span>
          </div>
        </div>
        {/* <Tabs
        ...
        <div className="min-h-screen bg-white from-slate-50 to-blue-50">
          <div className="max-w-full mx-auto">
            ...
            <div className="p-3">
              ...
              {allContent?.length != 0 && (
                <QuotaSummaryBar
                  remainingActive={statsRecord?.contentSummary?.totalApprovedNotShared}
                  remainingTotal={statsRecord?.contentSummary?.totalContent}
                  enActive={statsRecord?.contentSummary?.contentByLanguage?.english?.totalApprovedNotShared}
                  enTotal={statsRecord?.contentSummary?.contentByLanguage?.english?.count}
                  cnActive={statsRecord?.contentSummary?.contentByLanguage?.chinese?.totalApprovedNotShared}
                  cnTotal={statsRecord?.contentSummary?.contentByLanguage?.chinese?.count}
                  myActive={statsRecord?.contentSummary?.contentByLanguage?.malay?.totalApprovedNotShared}
                  myTotal={statsRecord?.contentSummary?.contentByLanguage?.malay?.count}
                />
              )}
            </div>
          </div>
